...
// .wrangler/tmp/bundle-UWIQie/checked-fetch.js
...
// .wrangler/tmp/pages-SNUaZp/functionsWorker-0.3442310029731357.mjs
...
Ich kann sehen, dass wir bereits ${messages.length - 1} Nachrichten ausgetauscht haben.`;
    ...
    const shouldGenerateCalendar = message.toLowerCase().includes("kalender") || message.toLowerCase().includes("termin") || message.toLowerCase().includes("meeting");
    if (shouldGenerateCalendar) {
      aiResponse += `

Hier ist ein Beispiel-Kalendereintrag f\xFCr dich:
...
async function onRequest2(context) {
  ...
  try {
    ...
    if (!prompt) {
      return jsonResponse2({ error: "Missing prompt" }, 400);
    }
    return jsonResponse2({ reply: `Echo: ${prompt}` });
  } catch (err) {
...
async function onRequest6(context) {
  ...
  try {
    ...
    const { action, data } = body;
    if (!action) {
      return errorResponse2("Missing 'action' field");
    }
    let result;
    switch (action) {
      case "save":
        if (!data?.prompt) {
          return errorResponse2("Missing 'prompt' in data");
        }
        result = await saveToAirtable4(env, data);
        break;
      default:
        return errorResponse2(`Unsupported action: ${action}`);
    }
    return new Response(JSON.stringify({ success: true, data: result }), {
      status: 200,
...
async function saveToAirtable4(env, data) {
  const { prompt, botAnswer, files = [], fileAttachments = [] } = data;
  const AIRTABLE_API_KEY = env.AIRTABLE_API_KEY;
  const AIRTABLE_BASE_ID = env.AIRTABLE_BASE_ID;
  const AIRTABLE_TABLE_NAME = env.AIRTABLE_TABLE_NAME || "Prompts";
...
__name(saveToAirtable4, "saveToAirtable4");
__name2(saveToAirtable4, "saveToAirtable");
async function onRequest7(context) {
  ...
  try {
    ...
    const { action, data } = body;
    if (!action) {
      return errorResponse3("Missing 'action' field");
    }
    let result;
    switch (action) {
      case "save":
        if (!data?.prompt) {
          return errorResponse3("Missing 'prompt' in data");
        }
        result = await saveToAirtable5(env, data);
        break;
      default:
        return errorResponse3(`Unsupported action: ${action}`);
    }
    return new Response(JSON.stringify({ success: true, data: result }), {
      status: 200,
...
async function saveToAirtable5(env, data) {
  const { prompt, botAnswer, files = [], fileAttachments = [] } = data;
  const AIRTABLE_API_KEY = env.AIRTABLE_API_KEY;
  const AIRTABLE_BASE_ID = env.AIRTABLE_BASE_ID;
  const AIRTABLE_TABLE_NAME = env.AIRTABLE_TABLE_NAME || "Prompts";
...
__name(saveToAirtable5, "saveToAirtable5");
__name2(saveToAirtable5, "saveToAirtable");
var routes = [
  {
    routePath: "/ai",
    mountPath: "/",
...
// .wrangler/tmp/bundle-UWIQie/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-UWIQie/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.3442310029731357.js.map
